...
- **`commands/`** — Tauri commands, organized by **macro-section** (mirrors the sidebar; grows with future sections). `commands/mod.rs` holds shared type aliases (`Database`/`Key`/`Runtime` = the managed `Arc`s) + `DEFAULT_LIMIT` and **re-exports every command with `pub use`** so `lib.rs` keeps referring to them as `commands::<name>`:
  ...
  - `commands/tools/{ports}.rs` — the Tools section's backend commands (today `list_ports`/`kill_process` for the Port Killer; FFI confined and balanced, errors via `AppError`).
...
- ~19 tools today (Port Killer, JSON/YAML/.env/HTML-entities/Base64-URL/case/slug converters, JWT, Regex, Cron, Timestamp, Number base, Generators, Fake data, QR, Hash compare, Markdown). Each tool is autonomous in its folder and never imports from another tool.
