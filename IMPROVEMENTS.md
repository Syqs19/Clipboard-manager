...
- **Quality tooling** — no ESLint, no CI quality-gate (CI runs only on `v*` release tags; the 83 Rust tests + 4 frontend tests + `tsc` never run on push/PR). Vitest is now set up, but it's not wired into CI yet.
