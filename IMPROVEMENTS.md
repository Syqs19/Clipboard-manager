...
- ✅ **Image Converter tool** — converts images between PNG, JPEG, WebP, AVIF, BMP, TIFF, ICO (input auto-detected) with quality (lossy formats), optional resize (max side, Lanczos3), and before→after size feedback (`humanBytes`). Local only via the `image` crate (made a direct dep, already transitive via arboard; `avif` feature pulls the AV1 encoder). Backend `commands/tools/convert.rs`: pure `convert_bytes` core (6 unit tests) + `convert_image_bytes_to_path` (single file: UI passes bytes + chosen dest, Rust writes it — no filesystem plugin needed) + `convert_images_batch` (multiple native paths → one output folder, best-effort per-file report). UI `tools/image-converter/`: drop/choose one image (bytes) or choose many (batch). Raster↔raster only.

- ✅ **Vectorial tool** — traces a raster image into a scalable **SVG** (raster→vector) via `vtracer`. Kept as a **separate tool** from the Image Converter on purpose: it's a reconstruction, not a format conversion — great on logos/icons/flat graphics, poor on photos (the UI says so). Options: black & white mode and a denoise (speckle) threshold. Backend `commands/tools/vectorize.rs`: pure `vectorize_bytes` core (3 tests) + `vectorize_image_to_path` (Rust writes the SVG). Dependency note: `vtracer` pulls its own old `image 0.23`, isolated by Cargo from our `0.25`; we only pass it raw RGBA bytes (decoded with our `image`), never a typed image, so there's no version clash. UI `tools/vectorial/`.

- ✅ **Tool search** — a search bar atop the Tools dashboard filters tools by name, description **and keywords**. New optional `keywords?: string[]` on `ToolDescriptor` (single source of truth in the registry — no separate format→tool map): so typing "png" surfaces every tool that handles PNG, including Vectorial (which doesn't mention PNG in its name/description). Multi-term AND ("convert yaml" narrows), case-insensitive. Empty query keeps the normal dashboard (favorites + drag-to-reorder); typing switches to a flat filtered grid with an empty state. `matchesQuery` is exported and unit-tested (6 tests against the real registry).
